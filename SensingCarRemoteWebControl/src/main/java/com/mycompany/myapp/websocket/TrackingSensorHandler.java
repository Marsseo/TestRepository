...
public class TrackingSensorHandler extends TextWebSocketHandler implements ApplicationListener{
	...
	public void init(){
		...
		coapClient.setURI("coap://192.168.3.54/trackingsensor");
		coapObserveRelation = coapClient.observe(new CoapHandler() {
			...
			public void onLoad(CoapResponse response) {
				...
				String status = jsonObject.getString("tracking");
				int state;
				if(status.equals("white")) state= 0;
				else state=1;

				jsonObject.put("time", getUTCTime(new Date().getTime()));
				jsonObject.put("tracking", state);
